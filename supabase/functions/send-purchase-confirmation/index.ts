...
<head>
  ...
  <title>Order Confirmation - VitalCanine</title>
</head>
  ...
  <div style="max-width: 600px; margin: 0 auto; padding: 40px 20px;">
    ...
    <div style="background: #ffffff; border-radius: 16px; padding: 40px; box-shadow: 0 4px 20px rgba(61, 42, 10, 0.08);">
      ...
      <div style="text-align: center; margin-top: 32px;">
        <a href="https://vitalcanine.lovable.app/account" style="display: inline-block; background: linear-gradient(135deg, #d4a853 0%, #c49942 100%); color: #1a1a1a; padding: 14px 32px; border-radius: 8px; text-decoration: none; font-weight: 600; font-size: 16px;">
          View My Orders
        </a>
      </div>
    </div>
    ...
    const res = await fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({
        ...
        subject: data.isSubscription 
          ? "🔄 Subscription Confirmed - VitalCanine" 
          : "✓ Order Confirmed - VitalCanine",
        html,
      }),
    });
